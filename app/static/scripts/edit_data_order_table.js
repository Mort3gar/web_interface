...
async function set_data_in_form(choice_id, input_values, keys_from_db) {
    ...
    SetInputs(input_values["input"], true, data_values);
     ...
     await get("/api/get_brands").then(data =>{data
        let index_counter = 0;
...
set_data_in_form("choice_brand", {"select": ["choice_brand"], "input": ["product_name", "model_name", "text_specification",
 "garantity_period", "client_name", "telephone_number", "date_order_acceptance"]}, ["product_name", "model_name",
 "text_specification", "warranty_period", "client_name", "phoneNumber", "order_receipt_date"])

async function add_order_in_db(){

    let data = get_data_from_input({"select": {"brand_name": "choice_brand"}, 
        "field": {"product_name": "product_name", "model_name":"model_name",
        "text_specification":"text_specification", "warranty_date":"garantity_period", "client_name":"client_name", 
        "telephone_number":"telephone_number", "date_order_acceptance":"date_order_acceptance"}});
    ...
    await post("/api/add_client", {"name": data["client_name"], "phoneNumber": data["telephone_number"]}).then(res =>{
        console.log(res)
    }).catch(error => {
        console.log(error)
    })
    ...
    await patch("/api/edit_product", {"id": product_id,"name": data["product_name"], "brands_id": brand_id, "model":data["model_name"],
    ...
    console.log({"clients_id": client_id, "product_id": product_id,
        "order_receipt_date": data["date_order_acceptance"]})

    // Формат отправляемых данных ['clients_id', 'product_id', 'order_receipt_date']
    await patch("/api/edit_order", {"clients_id": client_id, "product_id": product_id,
